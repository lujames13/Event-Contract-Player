...
| 階段 | 預估延遲 (ms) | 來源 / 備註 |
|------|-------------|------------|
| 1. 市場數據接收 (WS/REST) | 100 - 150 | 從倫敦 Origin 到台灣本地監控 |
| 2. 特徵工程 + 模型推理 | 20ms - 50ms | 基於現有 LightGBM/CatBoost 策略效能 |
| 3. 信號傳輸至倫敦 Relay | 110 - 130 | 台灣 -> 倫敦 單程網路延遲 |
| 4. 交易簽署與提交 (Relay) | 89.56ms | GCP London VPS 實測 (RTT) |
| 5. CLOB API 訂單確認處理 | 50 - 100 | Polymarket 內部處理時間 |
| **總計 (E2E Latency)** | **360ms - 490ms** | 從市場動態到訂單提交成功 |
