...
## 2. [SUSPENDED — Binance EC] Event Contract 規格
...
| 交易執行地 | 台灣本地（via Webshare Japan Proxy） | Proxy 穿越 geoblock，不再需要 VPS（G4.2 驗證） |
...
## 11. Proxy 與認證決策（2026-02-27）

| 決策 | 值 | Rationale |
|------|-----|-----------|
| Geoblock 繞過方式 | Webshare Japan Proxy（HTTP_PROXY / HTTPS_PROXY） | G4.2 驗證：本地直連 Polymarket L2 API 獲得 200 OK，免除 VPS 維護成本 |
| API Key 取得方式 | Runtime 自動衍生 (`client.derive_api_key()`) | 手動填寫的 API Key 與 Proxy Wallet (Funder) 簽名不匹配；動態衍生確保合法有效 |
| ClobClient 簽名模式 | `signature_type=2`（Proxy Wallet） | Phantom 產生的 Proxy Wallet 需要 Level 2 簽名，否則觸發 401 |
| VPS 需求 | **取消** | Proxy 方案完全取代原定的 GCP Tokyo VPS 部署 |