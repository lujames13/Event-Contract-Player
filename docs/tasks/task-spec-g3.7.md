...
### 修改範圍檢查
- [x] 僅修改了 `scripts/polymarket/verify_significance.py` (新增) 及相關文件（task spec, report）。
- [x] 未動到不可修改之核心檔案。

### 擴展測試摘要
在 `tests/review/specg3.7/test_significance_logic.py` 中實作了以下模擬測試：
- **門檻驗證**：驗證 N >= 200, DA p-value < 0.05, PnL p-value < 0.05 時是否正確標記為 `🟢 [GATE 3 PASSED]`。
- **不足數據驗證**：驗證 N < 200 時是否標記為 `⏳ [WAITING]`。
- **統計顯著性驗證**：驗證當勝率或 PnL 不具顯著性時（即使均值為正），p-value 是否正確計算且狀態維持 `⏳ [WAITING]`。
- **空資料處理**：驗證 DB 無資料時腳本是否優雅退場。
