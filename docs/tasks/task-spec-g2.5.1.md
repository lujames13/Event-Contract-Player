...
### 2026-02-21 17:17 (UTC+8)
- **執行人**: 使用者
- **操作項目**: 遷移至 GCP Japan (`asia-northeast1-b`) 重新執行驗證
- **結果**: 🟢 **SUCCESS (Access OK)** / 🟡 **Marginal (Latency High)**
  - **Geoblock**: `blocked: false` (日本 GCP IP 合法存取)
  - **CLOB Latency**: `p50: 329.06ms`, `p95: 507.41ms` (顯著高於歐洲，因地理距離)
  - **L1 Auth**: `HTTP 405` (認證行為與之前一致，需後續排查)
  - **Markets**: `0 markets` (仍讀取不到市場清單，需確認 API path)
- **結論**: 日本 GCP Datacenter 可規避 Geoblock，但延遲在 500ms 左右。
- **後續建議**: 若以 5m+ 策略為主，此延遲勉強可接受。報告已更新為日本實測數據。